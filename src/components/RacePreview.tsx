...
export function RacePreview({ race: initialRace, onClose, onRun }: Props) {
  ...
  // 2. Run the "Smart Allocation" logic globally for the week
  // This prevents the "Clone Horse" bug where a horse appears in multiple race previews
  const allocations = useMemo(() => {
    if (!allHorses || allHorses.length === 0) return {};
    return autoAllocateHorses(allHorses, weeklyRaces);
  }, [allHorses, weeklyRaces]);

  if (!allHorses || !teams) return null;

  // 3. Extract data for the CURRENTLY selected tab
    ...
    <div style={modalOverlay}>
      <div style={modalContent}>
        ...
        {weeklyRaces.length > 1 && (
            <div style={{ display: 'flex', backgroundColor: '#34495e', padding: '10px 10px 0 10px', gap: '5px', overflowX: 'auto', borderBottom: '1px solid #2c3e50' }}>
                {weeklyRaces.map(r => {
                    ...
                    const runnerCount = (allocations[r.id]?.field?.length || 0);
                    return (
                        <button 
                            key={r.id}
        ...
        <div style={{ backgroundColor: '#2c3e50', color: 'white', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: '12px', opacity: 0.8 }}>{activeRace.location} • {activeRace.distance}m • {activeRace.surface}</div>
            <h2 style={{ margin: 0 }}>{activeRace.name}</h2>
            {excluded.length > 0 && (
          ...
          <div style={{ backgroundColor: activeRace.grade === 'G1' ? '#3498db' : '#2ecc71', padding: '5px 10px', borderRadius: '4px', fontWeight: 'bold' }}>{activeRace.grade}</div>
        </div>
        ...
        <div style={{ padding: '20px', borderTop: '1px solid #eee', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
          ...
          {onRun && <button onClick={onRun} style={{ backgroundColor: '#27ae60', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}>🏁 Run Week</button>}
        </div>
      </div>
    </div>
  );
