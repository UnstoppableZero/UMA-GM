...
export function RosterPage() {
  ...
  return (
    <div>
      {/* HEADER */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', paddingBottom: '15px', borderBottom: '2px solid var(--border-default)' }}>
        <div>
          ...
          <div style={{ marginTop: '5px' }}>
            <label style={{ cursor: 'pointer', userSelect: 'none', fontSize: '14px', color: 'var(--text-secondary)' }}>
              <input 
                type="checkbox" 
                checked={showRetired} 
      ...
      {/* ROSTER TABLE */}
      <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'var(--bg-surface)', boxShadow: '0 1px 3px rgba(0,0,0,0.3)', fontSize: '14px', border: '1px solid var(--border-default)' }}>
        <thead style={{ backgroundColor: 'var(--bg-elevated)', color: 'var(--text-secondary)', borderBottom: '2px solid var(--border-default)' }}>
          <tr>
          ...
          {displayedRoster.map(uma => (
            ...
            >
              ...
              <td style={{ padding: '12px' }}>
                <Link to={`/uma/${uma.id}`} style={{ textDecoration: 'none', color: 'var(--text-primary)', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <td style={{ textAlign: 'center' }}>
                <span style={{ 
                  ...
                  backgroundColor: 'var(--bg-elevated)', padding: '4px 8px', borderRadius: '4px'
                }}>
                  {calculateOVR(uma)}
                </span>
              </td>

              <td style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>{uma.age}</td>
              
              <td style={{ textAlign: 'center', fontSize: '12px' }}>
                <div style={{ fontWeight: 'bold', color: 'var(--text-primary)' }}>{uma.career?.wins || 0} Wins</div>
...
const thStyle = { padding: '10px', textAlign: 'left' as const, fontWeight: '600', fontSize: '12px', textTransform: 'uppercase' as const, color: 'var(--text-secondary)' };
...
function renderGrade(val: number) {
  ...
  if (val >= 1200) { grade = 'SS'; color = '#e056fd'; }
  else if (val >= 1000) { grade = 'S'; color = '#f1c40f'; }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <span style={{ color: 'var(--text-muted)', fontSize: '10px', fontWeight: 'bold' }}>{Math.floor(val)}</span>
    </div>
  );
}
